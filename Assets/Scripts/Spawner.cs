...
public class Spawner : MonoBehaviour
{
    ...
    /// </summary>
    private float nextSpawnIn = 0.5f;
    
    private int consecutiveSpawns = 0;
    private bool lastSpawnDown = false;
    ...
    public void SpawnObstacle()
    {
        ...
        bool spawnDown;
        if (consecutiveSpawns >= 3)
        {
            // Force spawn on the opposite side if there were 3 or more consecutive spawns on the same side.
            spawnDown = !lastSpawnDown;
            consecutiveSpawns = 0;
        }
        else
        {
            spawnDown = RandomBool();
            if (spawnDown == lastSpawnDown)
            {
                consecutiveSpawns++;
            }
            else
